...
import { AuthService, type AppRole } from './auth.service';
import { MOCK_ACTIVITY, MOCK_QUERIES, MOCK_RESULTS, MOCK_USERS, type MockActivity } from './mock-data';

// This build ships without a backend: every method below resolves from the in-memory
...
// reload. The method signatures match the real client one-for-one, so swapping this
// service's bodies for HTTP calls is the only change needed to point at a live service.
...
export class MonitoringService {
  ...
  private queries: MonitoringQuery[] = MOCK_QUERIES.map((q) => ({ ...q }));
  private results: MonitoringResult[] = MOCK_RESULTS.map((r) => ({
    ...r,
    rows: r.rows.map((rw) => ({ ...rw })),
  }));
  private activity: MockActivity[] = MOCK_ACTIVITY.map((a) => ({ ...a }));
  private users: AppUser[] = MOCK_USERS.map((u) => ({ ...u, roles: [...u.roles] }));

  private nextMsorId = Math.max(...this.queries.map((q) => q.msorId)) + 1;
  private nextResultId = Math.max(...this.results.map((r) => r.resultId)) + 1;
  private nextRowId = Math.max(...this.results.flatMap((r) => r.rows.map((rw) => rw.rowId)), 0) + 1;
  private nextActivityId = Math.max(...this.activity.map((a) => a.activityId), 0) + 1;

  /** Who a mutation is attributed to. Mirrors the server reading the caller's token. */
  private author(): { name: string | null; email: string | null } {
    const me = this.auth.getCurrentUser();
    return { name: me?.name ?? null, email: me?.email ?? null };
  }

  fetchQueries(active?: 'Y' | 'N'): Promise<MonitoringQuery[]> {
    return delay().then(() =>
      active ? this.queries.filter((q) => q.activeYn === active) : [...this.queries],
    );
  }
  ...
  fetchResults(
    ...
    return delay().then(() => {
      ...
      let filtered = this.results.filter((r) => r.msorId === msorId);
   ...
   */
  triggerRun(msorId: number, window?: RunWindow): Promise<void> {
    return delay(800).then(() => {
      const query = this.queries.find((q) => q.msorId === msorId);
      ...
      const priorRows = this.results.filter((r) => r.msorId === msorId).flatMap((r) => r.rows);
  ...
  updateQuery(msorId: number, payload: QueryUpdatePayload): Promise<void> {
    return delay().then(() => {
      const idx = this.queries.findIndex((q) => q.msorId === msorId);
      if (idx === -1) throw new Error(`Query ${msorId} not found`);
      const defined = Object.fromEntries(
        Object.entries(payload).filter(([, v]) => v !== undefined),
      ) as Partial<MonitoringQuery>;
      this.queries[idx] = { ...this.queries[idx], ...defined, updatedAt: nowStamp() };
    });
  }

  createQuery(payload: QueryCreatePayload): Promise<MonitoringQuery> {
    return delay().then(() => {
      const now = nowStamp();
      const newQuery: MonitoringQuery = {
        msorId: this.nextMsorId++,
        title: payload.title,
  ...
  deleteQuery(msorId: number): Promise<void> {
    return delay().then(() => {
      this.queries = this.queries.filter((q) => q.msorId !== msorId);
      this.results = this.results.filter((r) => r.msorId !== msorId);
      this.activity = this.activity.filter((a) => a.msorId !== msorId);
    });
  }

  setOnHold(msorId: number, onHold: boolean): Promise<void> {
    return delay().then(() => {
      const idx = this.queries.findIndex((q) => q.msorId === msorId);
      if (idx === -1) throw new Error(`Query ${msorId} not found`);
      this.queries[idx] = {
   ...
   */
  fetchAllResultRows(filters: AllCasesFilters = {}): Promise<AllCasesRow[]> {
    return delay().then(() => {
      const byQuery = new Map(this.queries.map((q) => [q.msorId, q]));
      ...
      let rows: AllCasesRow[] = [...latest.entries()].map(([key, { row, result }]) => {
        ...
        return {
          ...
          activityCount: this.activity.filter(
      ...
      return rows.sort(
        (a, b) => b.runDate.localeCompare(a.runDate) || a.caseKey.localeCompare(b.caseKey),
      );
    });
  }
   ...
   */
  fetchCaseWorkload(month?: string): Promise<CaseWorkloadResponse> {
    return delay().then(() => {
      const months = this.activity.map((a) => a.createdAt.slice(0, 7)).sort();
      ...
      const inMonth = this.activity.filter(
        (a) =>
          a.createdAt.slice(0, 7) === target &&
          (a.entryType === 'COMMENT' || a.entryType === 'STATUS_CHANGE'),
      );

      const me = this.auth.getCurrentUser()?.email?.toLowerCase() ?? null;
  ...
  updateRow(rowId: number, rowStatus: RowStatus, rowComment: string | null): Promise<void> {
    return delay().then(() => {
      for (const result of this.results) {
        ...
        if (row.rowStatus !== rowStatus) {
          ...
          this.bumpActivityCount(result.msorId, row.caseKey);
        }
  ...
  private bumpActivityCount(msorId: number, caseKey: string): void {
    for (const result of this.results) {
  ...
  private toCaseActivity(a: MockActivity): CaseActivity {
    return {
  ...
  fetchCaseActivity(msorId: number, caseKey: string): Promise<CaseActivity[]> {
    return delay(150).then(() =>
      this.activity
        .filter((a) => a.msorId === msorId && a.caseKey === caseKey)
        .sort((a, b) => a.createdAt.localeCompare(b.createdAt))
        .map((a) => this.toCaseActivity(a)),
    );
  }

  addCaseComment(msorId: number, caseKey: string, text: string): Promise<CaseActivity> {
    return delay(200).then(() => {
      ...
      this.bumpActivityCount(msorId, caseKey);
      return this.toCaseActivity(entry);
    });
  }

  fetchUsers(): Promise<AppUser[]> {
    return delay().then(() => this.users.map((u) => ({ ...u, roles: [...u.roles] })));
  }
    ...
    return delay().then(() => {
      ...
      if (existing) {
        return {
          ...
          inviteUrl: `${window.location.origin}${this.baseHref}`,
          alreadyInvited: true,
        };
      }
  ...
  updateUserRoles(email: string, roles: AppRole[]): Promise<AppUser> {
    return delay().then(() => {
      const idx = this.users.findIndex((u) => u.email.toLowerCase() === email.toLowerCase());
      if (idx === -1) throw new Error(`No user ${email}`);
      if (this.users[idx].bootstrapAdmin) {
        throw new Error('This user is an administrator by configuration and cannot be edited here.');
      }
      this.users[idx] = { ...this.users[idx], roles: [...roles] };
      return { ...this.users[idx], roles: [...this.users[idx].roles] };
    });
  }
  ...
  deactivateUser(email: string): Promise<void> {
    return delay().then(() => {
      ...
      if (this.users[idx].bootstrapAdmin) {
        throw new Error(
          'This user is an administrator by configuration and cannot be removed here.',
        );
      }
      this.users[idx] = { ...this.users[idx], active: false };
    });
  }
}
